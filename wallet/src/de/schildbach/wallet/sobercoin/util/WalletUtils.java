...
import com.google.sobercoin.core.AddressFormatException;
import com.google.sobercoin.core.DumpedPrivateKey;
import com.google.sobercoin.core.ECKey;
import com.google.sobercoin.core.ScriptException;
import com.google.sobercoin.core.Sha256Hash;
import com.google.sobercoin.core.Transaction;
import com.google.sobercoin.core.TransactionInput;
import com.google.sobercoin.core.TransactionOutput;
...
public class WalletUtils
{
	...
	public static void writeKeys(final Writer out, final List<ECKey> keys) throws IOException
	{
		...
		out.write("# KEEP YOUR PRIVATE KEYS SAFE! Anyone who can read this can spend your Sobercoins.\n");
    ...
    public static SobercoinURI parseAddressString(String addressString) {
        if (addressString == null) return null;
        Log.d("Sobercoin", "Parsing: " + addressString);
        try {
            final SobercoinURI uri = new SobercoinURI(Constants.NETWORK_PARAMETERS, addressString);
            Log.d("Sobercoin", "URI: " + uri.getAddress().toString() + " " + uri.getLabel() + " " + uri.getAmount());
            return uri;
        }
        catch (final SobercoinURIParseException x)
        {
            // Try prepending sobercoin:
            try {
                final SobercoinURI uri = new SobercoinURI(Constants.NETWORK_PARAMETERS, "sobercoin:" + addressString);
                Log.d("Sobercoin", "URI: " + uri.getAddress().toString() + " " + uri.getLabel() + " " + uri.getAmount());
                return uri;
            }
            catch (final SobercoinURIParseException y)
            {
                return null;
            }
        }
